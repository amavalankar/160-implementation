## MicroMile Time-Distance conversion calculator

This system is a time-distance calculator designed for rental electric micromobility vehicles. Using this tool, users can compare travel distances and times for the different vehicles. The user can select a transportation mode and view the calculated distance or time for each transportation. The user can calculate the time from distance by inputting the desired travel time in minutes, and the system will show the estimated distance each transportation mode (including walking) will travel in the given time. Likewise, the user can calculate the distance from time by inputting the travel distance in miles and each transportation mode will show the travel duration in minutes. The system also informs the user if the miles inputted or calculated are out of range for a specific vehicle by highlighting those selections in red. 
